{
  ...
  in if (length configList) > 0 then let
    nextList = if (length configList) > 1 then
      lib.sublist 1 100 configList
    else [];

    nextNixvim = thisNixvim.nixvimExtend (elemAt configList 0);
  in extendWithList { thisNixvim = nextNixvim; configList = nextList; }
  else thisNixvim;
  ...
  extendWithNameList = thisNixvim: nameList: let
    ...
    getConfig = n: let
      fixPath = p: concatStringsSep "/" (
        lib.sublist 4 10 (
          lib.splitString "/" (
            toString p)));

      path = (get-snowfall-file "modules/nixos/${n}");
      try = if pathExists path then tryEval (import path)
        else (lib.warn "Nixvim config not found: ${fixPath path}" 
          {success = false; value = "noPath";});
    in
      if try.success then try.value
      else if try.value != "noPath" then
          (lib.warn "Nixvim config failed to import: ${fixPath path}" null)
        else null;
